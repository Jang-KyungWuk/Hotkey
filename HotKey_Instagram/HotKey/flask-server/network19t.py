...
def network(plaintext,ldaResult, sep='HOTKEY123!@#',
    ...
    try:
        start = time.time()
        print("시간 측정 시작")
        sdata = pp.plain_structurize(plaintext=plaintext, sep=sep,lineSplit=lineSplit)
        
        end = time.time()
        print("구조화 완료 in %s sec"%(end-start))
        start=end
        ...
        print("형태소 분석기 생성 완료 in %s sec"%(end-start))
        start=end
        ...
        print("pyvis 시각화 완료 in %s sec"%(end-start))
        start=end
